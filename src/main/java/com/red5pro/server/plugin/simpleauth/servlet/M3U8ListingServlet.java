	...
	public void doGet(HttpServletRequest req, HttpServletResponse resp) throws IOException {
		...
		if (appCtx == null) {
			...
			if (useAuth) {
				...
				log.info("Parameters - username: {} password: {} token: {} type: {} streamName: {}", username, password,
						token, type, streamName);
				if (StringUtils.isNotBlank(token)
					...
					try {
						...
						if (validator instanceof RoundTripAuthValidator) {
							...
							JsonObject result = ((RoundTripAuthValidator) validator).authenticateOverHttp(type,
									username, password, token, "*"); // wildcard the stream
			...
			if (useCloud) {
				...
				if (serviceS3 != null) {
					...
					for (int i = 0; i < length; i++) {
						...
						out.write(String.format(JSON_ENTRY_TEMPLATE, res.getFilename(), res.lastModified(),
								res.contentLength(), res.getURL()).getBytes());
						// comma or not to comma that is the question
						if (i < (length - 1)) {
							out.write(COMMA);
						}
